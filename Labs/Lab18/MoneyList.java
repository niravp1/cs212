...
public class MoneyList {
	...
	public void append(Money data) {
		MoneyNode newNode = new MoneyNode(data); // create a ListNode with data data
		last.next = newNode; // last node point to new ListNode
		last = newNode; // last node is newNode now
	...
	public void prepend(Money data) {
		MoneyNode newNode = new MoneyNode(data); // create a ListNode with data data
		if (first.next == null) // handle empty linked list case
			last = newNode;
		newNode.next = first.next; // newNode point to 2nd ListNode(first.next)
		first.next = newNode; // first node point to new ListNode
		length++; // increase length
	...
	public double sum(){
		double total = 0;
		MoneyNode curr = this.first.next;
		while (curr != null)
		{
			if (curr.data instanceof Bill )
				total += ((Bill)curr.data).getValue();
			else
				{
					total += ((Coin) curr.data).getValue();
				}	
			curr = curr.next;
		}
